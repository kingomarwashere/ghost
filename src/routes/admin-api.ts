...
adminApi.get('/stats', async (c) => {
  ...
  const [totalReports, wazeReports, userReports, totalUsers, routesToday, wazeByType] = await Promise.all([
    ...
    c.env.DB.prepare("SELECT type, COUNT(*) as n FROM reports WHERE reporter_hash = 'waze' AND expires_at > ? GROUP BY type ORDER BY n DESC").bind(now).all(),
  ]);
  ...
  return c.json({
    ...
    waze_by_type: wazeByType.results,
  });
});

// GET /api/admin/waze?limit=100
